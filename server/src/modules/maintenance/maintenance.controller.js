...
const { serializeInventoryItem } = require('./inventory.controller')
...
const normalizeStatus = (value = 'in_progress') => {
...
const buildMaintenancePayload = async (req, existingRecord = null) => {
  const body = req.body
  const vehicleId = body.vehicleId || body.vehicle || existingRecord?.vehicle
  const vehicle = await getOwnedVehicle(vehicleId, req.user._id)

  if (!vehicle) {
    return { error: 'Select a vehicle from your store fleet' }
  }

  const status = normalizeStatus(body.status || existingRecord?.status || 'in_progress')
  ...
  const inventoryItem = inventoryItemId
    ? await getOwnedInventoryItem(inventoryItemId, req.user._id)
    : null
  ...
  const previousVehicleStatus = isActiveMaintenance(status)
    ? (
      sameVehicleAsExistingRecord && existingRecord.previousVehicleStatus
        ...
        : await getPreviousVehicleStatus(vehicle, req.user._id, existingRecord?._id)
    )
...
const getMaintenanceRecordById = async (req, res) => {
  try {
    const record = await Maintenance.findOne({ _id: req.params.id, owner: req.user._id })
      .populate(maintenancePopulate)
    ...
    return res.json(serializeMaintenanceRecord(record))
  } catch (error) {
...
const createMaintenanceRecord = async (req, res) => {
  try {
    const { payload, error, inventoryItem } = await buildMaintenancePayload(req)
    ...
    await syncVehicleStatusForRecord(record, req.user._id)
...
const updateMaintenanceRecord = async (req, res) => {
  try {
    ...
    const { payload, error, inventoryItem } = await buildMaintenancePayload(req, record)
    ...
    await record.save()
    await syncVehicleStatusForRecord(record, req.user._id, previousVehicleId, previousVehicleStatus)
...
const deleteMaintenanceRecord = async (req, res) => {
  try {
    ...
    if (record.inventoryConsumed && record.inventoryItem) {
      await Inventory.updateOne(
        { _id: record.inventoryItem, owner: req.user._id },
        { $inc: { quantity: Number(record.count || 0) } }
      )
    }

    await record.deleteOne()
    await restoreVehicleIfClear(vehicleId, req.user._id, previousVehicleStatus)
