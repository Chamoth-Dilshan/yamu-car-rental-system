...
export default function AdminBookings() {
  ...
  useEffect(() => {
    ...
    API.get('/admin/bookings', { params: filters })
      ...
      .catch((err) => setError(err.response?.data?.message || 'Failed to load vehicle bookings'))
      .finally(() => setLoading(false))
  }, [filters, reloadKey])

  const updateBooking = async (bookingId, payload, successMessage, actionKey) => {
    const targetBooking = bookings.find((item) => item._id === bookingId)
    const actionLabel = payload.bookingStatus === 'confirmed'
      ? 'confirm'
      : payload.bookingStatus === 'completed'
        ? 'complete'
        : payload.bookingStatus === 'closed'
          ? 'close'
          : payload.bookingStatus === 'cancelled'
            ? 'cancel'
            : 'update'

    if (!confirmBookingStatusChange(targetBooking?.bookingNo || 'this booking', actionLabel)) {
      return
    }
  ...
  const deleteBooking = async (bookingId) => {
    if (!window.confirm('Delete this vehicle booking record?')) {
      return
    }
    ...
    try {
      ...
      await refreshNotifications().catch(() => {})
      setMessage('Vehicle booking deleted')
      setReloadKey((prev) => prev + 1)
    } catch (err) {
      setError(err.response?.data?.message || 'Failed to delete vehicle booking')
    } finally {
      setBusyAction('')
    }
  }
  ...
  const summaryCards = [
    { label: 'Total Vehicle Bookings', value: stats?.totalBookings || 0 },
    ...
    { label: 'Closed', value: stats?.closedCount || 0 }
  ]
    ...
    <div className="dashboard-layout page-content">
      ...
      <main className="dashboard-content">
        ...
        <section className="form-card">
          ...
          {loading ? (
            <div className="reservation-empty">Loading vehicle bookings...</div>
          ) : bookings.length > 0 ? (
                ...
                <tbody>
                  {bookings.map((booking) => (
                    <tr key={booking._id}>
                      ...
                      <td>
                        <strong>{booking.vehicle?.vehicleCode || 'No vehicle code'}</strong>
                      ...
                      <td>
                        <div className="table-actions">
                          ...
                          <button
                            className="btn btn-danger btn-sm"
                            type="button"
                            disabled={busyAction === `delete-${booking._id}`}
                            onClick={() => deleteBooking(booking._id)}
                          >
                            {busyAction === `delete-${booking._id}` ? 'Deleting...' : 'Delete'}
                          </button>
                        </div>
                      </td>
                    </tr>
            ...
            <div className="reservation-empty">No vehicle bookings matched the current filters.</div>
          )}
        </section>
      </main>
    </div>
